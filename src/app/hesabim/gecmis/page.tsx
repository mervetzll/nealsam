...
export default function GiftHistoryPage() {
  const [items, setItems] = useState<SavedGift[]>([]);
  ...
  async function deleteItem(id: string) {
    ...
    await supabase.from("saved_gift_results").delete().eq("id", id);
      ...
      <section className="mx-auto max-w-6xl px-5 py-14">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="mt-3 text-4xl font-black tracking-tight text-[#2b1b1b]">
              Kaydettiğin hediye önerileri
            </h1>
            <p className="mt-4 max-w-2xl text-sm leading-6 text-[#6b4a4a]">
              Hediye Bul sonucunda kaydettiğin öneriler burada görünür.
            </p>
          </div>
        ...
        {loading && (
          <div className="mt-10 rounded-[2rem] border border-pink-100 bg-white p-8 text-center shadow-sm">
            <p className="font-bold text-[#2b1b1b]">Geçmiş yükleniyor...</p>
          </div>
        )}
        ...
        {!loading && !loggedIn && (
          <div className="mt-10 rounded-[2rem] border border-pink-100 bg-white p-8 text-center shadow-sm">
            <h2 className="text-2xl font-black text-[#2b1b1b]">
              Geçmişi görmek için giriş yap
            </h2>
            <p className="mt-3 text-sm text-[#6b4a4a]">
              Kaydettiğin hediyeler hesabına bağlı tutulur.
            </p>
        ...
        {!loading && loggedIn && items.length === 0 && (
          <div className="mt-10 rounded-[2rem] border border-pink-100 bg-white p-8 text-center shadow-sm">
            <h2 className="text-2xl font-black text-[#2b1b1b]">
              Henüz kayıtlı önerin yok
            </h2>
            <p className="mt-3 text-sm text-[#6b4a4a]">
              Hediye Bul sayfasında beğendiğin sonucu hesabına kaydedebilirsin.
            </p>
